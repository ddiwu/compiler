...
void translate_main(CodeGen *codegen) {
    ...
    codegen->append_inst("andi", {"$t0", "$t0", "0"});
    codegen->append_inst("bcnez", {"$fcc0", ".skip_add"});
    codegen->append_inst("addi.w", {"$t0", "$t0", "1"});
    codegen->append_inst(".skip_add", ASMInstruction::Label);
